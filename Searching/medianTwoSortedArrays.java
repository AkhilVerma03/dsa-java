public class medianTwoSortedArrays {

    // OPTIMAL SOLUTION: Binary Search on Cut Index
    // Time Complexity: O(log(min(n1,n2)))
    // Space Complexity: O(1)
    public double optimal(int nums1[], int[] nums2) {
        if (nums1.length > nums2.length) {
            return optimal(nums2, nums1); 
        }
        int min1, min2, max1, max2;
        int n1 = nums1.length, n2 = nums2.length;
        int begin1 = 0, end1 = n1;
        while (begin1 <= end1) {
            int i1 =(begin1+end1)/2;
            int i2 = (n1 + n2 + 1) / 2 - i1;
            min1 = (i1==n1)?Integer.MAX_VALUE:nums1[i1];
            min2 = (i2==n2)?Integer.MAX_VALUE:nums2[i2];
            max1 = (i1==0)?Integer.MIN_VALUE:nums1[i1-1];
            max2 = (i2==0)?Integer.MIN_VALUE:nums2[i2-1];
            if(max1<=min2&&max2<=min1){
                if((n1+n2)%2==0)
                    return (Math.max(max1,max2)+Math.min(min1,min2))/2.0;
                else
                    return Math.max(max1,max2);
            }
            else if(max1>min2){
                end1=i1-1;
            }
            else{
                begin1 = i1+1;
            }

        }

        return 0.0;
    }


    // NAIVE SOLUTION: Two-Pointer Merge
    // Time Complexity: O(n1 + n2)
    // Space Complexity: O(n1 + n2)

   /*
    public double naive(int[] nums1, int[] nums2) {
    int[] merged = new int[nums1.length + nums2.length];
    int i = 0;
    int j = 0;
    int k = 0;
    while (i < nums1.length && j < nums2.length) {
    if (nums1[i] <= nums2[j]) {
    merged[k] = nums1[i];
    i++;
    } else {
    merged[k] = nums2[j];
    j++;
    }

    k++;
    }
    while (i < nums1.length) {
    merged[k] = nums1[i];
    i++;
    k++;
    }
    while (j < nums2.length) {
    merged[k] = nums2[j];
    j++;
    k++;
    }
    int n = merged.length;
    if (n % 2 == 1) {
    return merged[n / 2];
    }
    return (merged[n / 2] + merged[(n / 2) - 1]) / 2.0;
    } 
    */

}
